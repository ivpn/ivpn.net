<template>
    <div>
        ...
        <div v-if="lang == 'ua'">
            <p class="disclaimer">Надсилаючи цю форму, ви погоджуєтеся з тим, що ваша IP-адреса може бути збережена, щоб допомогти нам уникнути зловживань. IP-адреси, збережені таким чином, не використовуються для будь-яких інших цілей. Решта сайту IVPN регулюється нашою загальною <a href="/privacy/">політикою конфіденційності.</a></p>
            <p>Умови даної пропозиції:</p>
            <ul>
                <li>Не частіше одного запиту на користувача в місяць</li>
                <li>У випадку відправки численних запитів протягом місяця, ми зберігаємо за собою право закрити обліковий запис</li>
                <li>Тільки для нових користувачів IVPN</li>
            </ul>
        ...
        <div v-if="lang == 'ru'">
            <p class="disclaimer">Отправляя эту форму, вы соглашаетесь с тем, что ваш IP-адрес может быть сохранен, чтобы помочь нам избежать злоупотреблений. Сохраненные таким образом IP-адреса не используются для каких-либо других целей. Остальная часть сайта IVPN регулируется нашей общей <a href="/privacy/">политикой конфиденциальности</a>.</p>
            <p>Условия данного предложения:</p>
            <ul>
                <li>Не чаще одного запроса на пользователя в месяц</li>
                <li>В случае отправки множественных запросов в течение месяца, мы сохраняем за собой право закрыть учетную запись</li>
                <li>Только для новых пользователей IVPN</li>
            </ul>
